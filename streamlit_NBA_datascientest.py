...
st.set_page_config(layout = "wide", 
                   page_title = "Projet MVPy datascientest")
...
def read_file(emplacement):
    ...
    df = pd.read_csv(emplacement, sep = ",")
    
    return df


# Importation des données de tirs brutes, stockées dans 8 fichiers csv
link_1 = "https://github.com/JulienLickel/NBA-shots-predictions-/blob/main/Data/tirs_partie_2.csv"
...
def composition(datas):
    ...
    for i in range(len(datas)-1):
        data_elargi = pd.concat([data_elargi,datas[i+1]] , axis = 0)
        
    data_elargi = data_elargi.reset_index()

    data_elargi = data_elargi.drop("index" , axis = 1)
        
    data_elargi = data_elargi.drop(columns = ["Unnamed: 0"])
    
    return data_elargi
    

# version à 20 joueurs : 
    
datas = [df1, df2, df3]

df = composition(datas)
...
datas = [df1, df2, df3, df4, df5, df6, df7, df8]

df_elargi = composition(datas)
...
players = ["Tim Duncan", "Kobe Bryant" ,"Allen Iverson", "Steve Nash", "Ray Allen", 
           "Paul Pierce", "Pau Gasol", "Tony Parker", "Manu Ginobili", "Dwyane Wade", 
           "LeBron James", "Chris Paul", "Kevin Durant", "Russell Westbrook", "Stephen Curry", 
           "James Harden", "Kawhi Leonard", "Damian Lillard", "Anthony Davis", 
...
def update(data=df):
    ...
    # Filtrage de data_copy : 
        
    data_copy = data_copy[data_copy["Player Name"].isin(players)]
    
    # Nettoyage de data_copy :

    data_copy = nettoyage_df(data=data_copy)
...
players = ['Tim Duncan', 'Kobe Bryant', 'Allen Iverson', 'Steve Nash', 'Ray Allen', 'Paul Pierce',
          ...
          'Damian Lillard', 'Anthony Davis', 'Giannis Antetokounmpo','Dirk Nowitzki' ,
          'Shaquille O\'Neal', 'Kyle Korver', 'Rajon Rondo', 'Danny Green', 'Kevin Garnett' ,
...
df_elargi = update(data = df_elargi)
...
if pages == "Petite initiation à la NBA":
    ...
    col1.dataframe(franchises, width = 600)
    ...
    st.write("")
    st.markdown("""Une autre particularité de la NBA est le fait que le **calendrier des matchs d'une franchise est asymétrique** : bien que chaque franchise joue le même nombre de matchs que les autres, le nombre de matchs joués contre chaque adversaire diffère d'une franchise à une autre.  
                   En effet, afin d'éviter des déplacements longs et coûteux permanents pour jouer des matchs à l'autre bout des Etats-Unis, les 30 franchises sont réparties **selon leur situation géographique** dans **2 conférences** (conférence **Ouest** et conférence **Est**, comptant chacune **15 franchises**) 
                   et dans **6 divisions** (chaque conférence comptant **3 divisions de 5 franchises**).  
                   Pour plus de clarté, voici une image résumant la répartition des 30 franchises actuelles :""",
                unsafe_allow_html=True)
    ...
    st.write("Il faut donc bien comprendre que les franchises **n'ont pas un calendrier équivalent** et que **la difficulté du calendrier dépend de la qualité de la division et de la conférence** à laquelle appartient la franchise en question, puisque ce découpage est basé uniquement sur un critère de proximité géographique et ne tient absolument pas compte du niveau de compétitivité des franchises.")

    st.write("Une saison NBA se déroule, depuis la saison 2020-2021, **en 3 parties** (au lieu de 2 auparavant) :")

    st.write(
        """
             - **PARTIE 1** : la **saison régulière**, lors de laquelle chaque franchise joue un total de **82 matchs** (**41** matchs à domicile et **41** matchs à l'extérieur) du mois  
                              d'**octobre** au mois d'**avril** de l'année suivante, selon le schéma suivant :
                     
                - **4 matchs** contre chacune des 4 équipes de sa propre division (pour un total de 4x4 = **16** matchs).
                -  **3 ou 4 matchs** contre chacune des 10 équipes de sa conférence mais qui n'appartiennent pas à sa franchise (pour un total de **36** matchs).
                - **2 matchs** contre chacune des 15 équipes de l'autre conférence (pour un total de 2x15 = **30** matchs).
              ...
              mène devant l'autre à l'issue d'une période de 5 minutes supplémentaires.  
               
              Ainsi, **chaque conférence possède son propre classement de saison régulière** dans lequel les équipes sont classées **selon leur ratio victoires/défaites** : autrement dit, la position au classement d'une franchise sera d'autant meilleure que cette franchise aura remporté de matchs.   
        """
    )

    st.write("")

    st.write("Voici par exemple le classement final de la saison régulière pour la conférence *Est* lors de la saison 2014/2015 (la colonne **%V** indique le pourcentage de matchs gagnés) :")
    ...
    st.write("""- **PARTIE 2** : les **Play-in**, introduites lors de l'exercice 2020-2021, par lesquelles doivent passer les franchises **classées de la 7ème place à la 10ème place de chaque conférence** : dans chaque conférence, le 7ème et le 8ème ainsi que le 9ème et le 10ème s'affrontent lors d'un seul match décisif, l'équipe ayant terminée à la meilleure position lors de la saison régulière recevant à domicile : celle des deux qui remporte ce match reste en lice dans la compétition.   
                  La franchise qui perd la série '9ème vs 10ème' est éliminée tandis que l'autre affrontera le perdant de la rencontre '7ème vs 8ème', afin de savoir quelle franchise jouera le prochain tour : les *Playoffs*.  
                  Deux franchises de chaque conférence sont donc éliminées lors de ce tour.""")

    file_play_in = "https://ca-times.brightspotcdn.com/dims4/default/51539fe/2147483647/strip/true/crop/784x441+0+0/resize/840x473!/quality/90/?url=https%3A%2F%2Fcalifornia-times-brightspot.s3.amazonaws.com%2F94%2F62%2F12d07ba543748b15c84583eb6cb7%2Ffull-play-in-tournament-bracket-16x9-1-784x441.png"
    ...
    st.write(
        """
             - **PARTIE 3 :** les **Playoffs**, auxquelles participent les équipes ayant terminées **aux 6 premières places de chaque conférence lors de la saison régulière** ainsi que **les 2 équipes qualifiées à l'issue des Play-in**.  
               Les Playoffs de jouent en **4 tours éliminatoires** dans chaque conférence, dans des séries de 7 matchs dont l'issue se décide "au meilleur des 7 matchs" : autrement dit, lors de chaque tour, la première des 2 équipes à remporter 4 matchs contre son adversaire désigné remporte la série et est qualifiée pour le prochain tour, tandis que l'autre est éliminée.  
               Voici le déroulé des Playoffs : 
             
                - le **1er tour**, lors duquel 4 équipes sont éliminées dans les 2 conférences (il en reste alors 4 dans chaque conférence à l'issue de ce 1er tour).
                ...
                - les **finales de conférence**, qui voient s'affronter les 2 derniers rescapés dans chaque conférence : le vainqueur de chaque finale est alors déclaré champion de sa conférence.
    ...
    st.write("""*Tim Duncan, Kobe Bryant, Allen Iverson, Steve Nash, Ray Allen, Paul Pierce, 
             ...
             la plupart de ces noms ne vous disent peut-être rien, et pourtant… **Ce sont 20 des meilleurs joueurs NBA du 21ème siècle**, selon le média américain ESPN.  
    ...
    st.write("""**Mais quelles sont (ou étaient) les habitudes et routines de tir de ces grands noms du basketball ?  
                  Et serions-nous capable de prévoir l’issue de leur tir (marqué ou raté) à partir du contexte dans lequel a été tenté ce tir ?**""")
    ...
    st.write("""Téléportons-nous un bref instant aux Etats-Unis, un pays où la NBA et l’analyse de données 
             ...
             Dans ce sport, [de nombreuses de franchises NBA possèdent leur propre équipe de data analysts et data 
             scientists](https://youtu.be/MpLHMKTolVw) dédiée à l’étude des performances des joueurs et des équipes adverses de la ligue : 
             **admettons que nous soyons nous-même membre de l’équipe d’analystes d’une franchise NBA.**  
             Lorsque surviennent des matchs face à des équipes dans lesquelles jouent des stars mondiales du 
             basketball telles que *Kevin Durant*, *LeBron James*, *James Harden*, etc…, redoutables en attaque et quasi-innarêtables quand elles le veulent, 
             mieux vaut avoir une bonne stratégie pour tenter de contrecarrer leurs plans et les empêcher de développer leur 
             jeu d’attaque à la perfection sous peine de concéder des dizaines et des dizaines de points uniquement de la part 
             de ces joueurs.""")

    st.write("""**L’entraîneur nous confie donc une mission :** à partir des données concernant les tirs de 10 des meilleurs joueurs 
    ...
    st.write("""
        ...
        - d’identifier d’éventuels contextes dans lesquels le joueur en question **excelle au tir**, afin **d’orienter 
          le choix du défenseur direct** à placer sur ce joueur, **le traitement défensif à appliquer** sur ce joueur 
          **voire même le placement d'un autre défenseur en soutien du premier.**
          
        - au contraire, d’identifier d’éventuels contextes dans lesquels le joueur **est en difficulté au tir**, 
          afin là aussi d’adapter la défense et éventuellement **pousser le joueur à attaquer puis tirer dans une zone et un 
          contexte “défavorables”** dans lesquels il se montre habituellement moins à l'aise et moins efficace, autrement dit 
          **dans lesquels sa probabilité de marquer est plus faible.**""")

    st.write("")

    st.header("Plan du projet.")

    st.write("""Ce projet **MVPy** est composé de 2 parties assez indépendantes : """)

    st.write("""
             
                **- PARTIE 1** (*analyse*) : identifier, décrypter et comparer les prises de tirs des 20 joueurs, 
                                             tant en termes de **proportion de tirs tentés** que de **réussite au tir** selon la zone du terrain.  
                              
                **- PARTIE 2** (*analyse* et *modélisation*) : restreindre la liste à seulement 9 joueurs toujours actifs aujourd'hui 
    ...
    st.write("""L'objectif ici est de comprendre les données sur lesquelles nous allons travailler, c'est-à-dire de faire des recherches 
                sur les différentes variables présentes, leur signification, leurs valeurs/modalités (grâce à la documentation de l'auteur du jeu de données et à internet), 
                faire des petites recherches sur le basketball et la NBA en général afin de s'acclimater aux données disponibles.""")

    st.markdown("#### 2. Nettoyage des données.")

    st.write("")
    st.write("""Lors de cette étape, l'objectif est de mettre en forme les données en fonction des besoins suscités par notre projet (filtrage) mais également pour une 
                manipulation plus simple et automatique, sans trop avoir à réfléchir (syntaxe des noms de colonnes et des modalités).  
                C'est également ici que l'on a créé et supprimé certaines variables du jeu de données initial.""")

    st.markdown("#### 3. (PARTIE 1) Comparaison des habitudes de tirs des 20 joueurs.")

    st.write("")
    st.write("""Grâce aux informations disponibles sur les coordonnées de chacun des tirs tentés par chaque joueur et à la création de fonctions permettant de tracer 
    ...
    st.markdown(
        "#### 4. (PARTIE 2) Analyses statistiques des tirs selon le poste pour les 9 joueurs encore actifs.")

    st.write("")
    st.write("""
             - **4-1) Analyse univariée.**  
    
                Pour chaque groupe de joueurs, nous cherchons à visualiser et analyser les tirs tentés au travers d'un seul caractère.
             
                
             - **4-2) Analyse bivariée.**  
    
                Pour chaque groupe de joueurs, nous cherchons à croiser 2 caractères et à visualiser les relations/associations possibles entre les variables.  
    ...
    st.markdown("#### 5. (PARTIE 2) Modélisation.")
    ...
    st.write("""
            ...
            à la variable cible (*'shot made flag'*).
            
            - **5-2) Réglage du jeu de données.**
            
            Cette étape consiste en la recherche des variables et des données à utiliser pour entraîner notre modèle afin 
            ...
            ne sur-apprend ni ne sous-apprend.  
            Pour cela, nous utilisons un modèle “bateau” d’arbre de 
            ...
            type de modèle, le sur-apprentissage sera tout aussi réduit.  
            Cette étape se décompose en **2 parties** :
            
            **.** *a) Sélection des variables.*  
              
               On recherche ici manuellement ou à l’aide d’outils de sélection de variables (*feature_importance* par exemple) une ou 
               plusieurs combinaisons de variables explicatives prometteuses, c’est-à-dire permettant au 
               modèle de ne pas sur-apprendre et d’avoir un bon compromis de base entre *recall* et *precision*.
            ...
            **.** *b) Recherche de données aberrantes.*  
              ...
              Cependant, très peu voire même parfois aucune donnée n’était à consédérer comme une donnée aberrante.  
              ...
              aberrantes scientifiquement et méthodologiquement plus acceptable."(*[extrait wikipédia](https://fr.wikipedia.org/wiki/Donn%C3%A9e_aberrante#Exclusion_de_la_donn%C3%A9e_aberrante)).  
              C’est pourquoi, dans ce projet, toutes les données ont été conservées hormis un faible 
              nombre de lignes lors de la suppression des types de tir très rares (variable “action 
              type) lors du nettoyage des données.  
              
            - **5-3) Recherche des types de modèles prometteurs.**  
            
            Nous entraînons et évaluons grâce à une procédure d'évaluation "personnelle" utilisant la validation croisée, 4 types de modèles de classification classiques : *random forest*, *KNN*, 
            *régression logistique* et *SVM* avec la combinaison de variables explicatives ainsi que les données 
            d’entraînement choisies en 5-1) et 5-2).  
            Suivant l'importance du sur-apprentissage et les performances de base en termes de *recall* et 
            *precision*, nous en déduisons quels sont les types de modèles qui valent le coup d’être 
            optimisés au niveau de leurs hyperparamètres.  
            
            - **5-4) Optimisation d’hyper paramètres par GridSearch.**  
            
            Nous tentons ensuite lorsque cela est possible (parfois extrêmement long pour le modèle de *SVM*, donc 
            n'aboutissant pas), de rechercher la meilleure combinaison d’hyperparamètres possible par 
            recherche par grille (*GridSearch*) pour chaque type de modèle sélectionné lors de l’étape 5-3).  
            
            - **5-5) Comparaison des types de modèles optimisés.**  
            
            Un fois optimisés, nous ré-entraînons et évaluons une dernière fois les types de modèles avec leurs hyperparamètres "optimaux"
            à l’aide de notre procédure d’évaluation, puis comparons leurs performances : le modèle choisi 
            sera celui présentant le meilleur compromis entre *recall* et *precision* et présentant le *recall* le 
            plus élevé (il est préférable ici d'avoir un modèle ayant un bon recall mais une precision moindre, plutôt qu'un modèle ayant un recall faible mais une bonne precision).  
            Si les résultats obtenus pour notre modèle après ces 5 étapes ne sont pas jugés suffisamment bons (jugement personnel), 
            les étapes 5-3/4/5) pourront être répétées avec une autre combinaison de 
            ...
            manuelle des variables explicatives, etc …).  
            
            - **5-6) Evaluation finale.**  
            
            Enfin, nous entraînons le modèle sélectionné avec la combinaison de variables explicatives choisie avec les hyperparamètres optimisés 
            sur le jeu d’entraînement tout entier, puis nous l’évaluons sur le jeu d'évaluation ayant été mis de côté depuis 
            le début et qui comprend des données jamais vues par le modèle : les performances obtenues par le modèle sur ce jeu de données sont ainsi un
            bon indicateur des performances réelles qu'aura notre modèle "dans la vraie vie" lorsqu'il sera déployé.
    ...
    st.write("""Le jeu de données que nous avont utilisé afin de réaliser notre projet est disponible et téléchargeable
             ...
             1997 et 2019.**  
             Etant donné la taille de ce fichier et la taille limite autorisée des dépôts sur GitHub, **seule la partie concernant les 20 joueurs du projet (plus quelques joueurs supplémentaires pour la partie "BONUS")" 
             a été importée dans cette application streamlit.**""")
    ...
    st.write("")
    curseur_data = st.slider("Sélectionnez le nombre de lignes à afficher :", 5, 500)

    st.dataframe(df.head(curseur_data))
    ...
    st.write("""Bien que déjà relativement propre (aucune valeur manquante ni aucun doublon), ce jeu de données
    ...
    st.write("""
            ...
            **- Mise en minuscules de toutes les modalités de toutes les colonnes**, hormis pour les colonnes *"Home Team"* et *"Away Team"*.  
    ...
    description_variable = np.array(["nom du joueur",
                                     "franchise à laquelle appartient l'équipe du joueur",
                                     "conférence à laquelle appartient l'équipe du joueur",
                                     "division à laquelle appartient l'équipe du joueur",
                                     "franchise à laquelle appartient l'équipe adverse",
                                     "conférence à laquelle appartient l'équipe adverse",
                                     "division à laquelle appartient l'équipe adverse",
                                     "date du match",
                                     "phase de la saison (saison régulière ou Playoffs)",
                                     "lieux du match (domicile ou extérieur)",
                                     "numéro du quart-temps en cours",
                                     "temps restant à jouer dans le quart-temps en cours",
                                     "coordonnée x du tireur",
                                     "coordonnée y du tireur",
                                     "distance séparant le panier du tireur (en ft)",
                                     "type de tir tenté",
                                     "valeur du tir tenté (2 points ou 3 points)",
                                     "zone du terrain dans laquelle se situe le tireur",
                                     "orientation du tireur par rapport au panier (dans l'axe, à gauche, excentré à gauche, etc...)",
                                     "catégorisation de 'shot distance'",
                                     "finalité du tir (marqué ou raté)"])
    ...
    st.write("""
             ...
             fréquence d'apparition est très faible, et que l'on ne voit que très peu sur un terrain NBA  (par exemple : 
             *'Running Alley Oop Layup Shot'* ou *'Driving Finger Roll Layup Shot'*).  
             ...
             - de même, la présence d'une modalité *'No Shot'*, qui apparaît 0.0241 % du temps est surprenante : il paraît étrange que le fait 
               ...
               parfois 0 (tir raté) et parfois 1 (tir réussi), ce qui est absurde et contradictoire.  
            ...
            Pour plus d'informations sur ces différents types de tirs, [cliquez sur ce lien](https://basket-ballworld.fr/les-differents-types-de-tirs-au-basket-ball/).  
    ...
    st.write("""
             - **"time remaining"** : les variables *"Minutes Remaining"* et *"Seconds Remaining"* ne sont pas vraiment exploitables en l'état, car le temps restant à jouer 
                 est divisé en 2 parties et stocké dans 2 colonnes distinctes.  
                 Il semble plus pratique d'avoir une seule colonne indiquant le temps restant sous la forme ***minutes + secondes*** dans une seule variable, ce qui réduit par la même occasion le nombre 
                 de variables explicatives de nos futurs modèles.  
                 
            - **"adversary"** et **"home"** : à l'origine, 3 variables étaient consacrées au stockage des noms d'équipes (*"Team Name"*, *"Home Team"* et *"Away Team"*) indiquant respectivement le nom de l'équipe pour laquelle 
              joue le tireur, le nom de l'équipe jouant à domicile et le nom de l'équipe jouant à l'extérieur.  
              Mais dans cette configuration, il nous est impossible de savoir :  
                  
                 1) Si le tireur joue à domicile/à l'extérieur, ce qui constitue une information intéressante.  
                 2) Face à quelle équipe tire ce joueur.  
        
             C'est pourquoi les variables *"home"* et *"adversary"* ont été créées afin de remplacer les variables *"Home Team"* et *"Away Team"*, sur la base desquelles elles ont d'ailleurs été construites.  
            
            - **"conference"**, **"division"**, **"conference adv"** et **"division adv"** : ces 4 variables ont été construites manuellement à l'aide de la connaissance de la répartition des différentes franchises 
              ...
              compétitivité plus ou moins élevé : savoir à quelle conférence et à quelle division appartient l'équipe du tireur mais aussi l'équipe adverse pourraient donc constituer des informations supplémentaires utiles
              à la détermination de l'issue du tir d'un joueur.  
            ...
            - **'shot distance'** : la variable indiquant la distance séparant le tireur du panier était déjà présente dans le jeu de données brutes, mais sa valeur était arrondie au dixième.  
              ...
              la distance tireur-panier, et les 2 autres côtés ont pour longueur la coordonnée en x du tireur (*"X Location"*) et la coordonnée en y du tireur (*"Y Location"*).
             """)

    st.write("")
    st.write("")

    st.write("Une fois appliquées toutes les opérations de nettoyage listées précédemment, **voici l'allure du jeu de données qui nous a servi par la suite pour réaliser les analyses et la modélisation :**")
    ...
    col2.dataframe(filtrage_data(df, bouton_joueur, bouton_variables, bouton_lignes))
    ...
    st.write("""
            Afin que les comparaisons effectuées aient un sens, nous avons choisi de **regrouper les 20 joueurs 
            de la liste selon le poste qu'ils occupent le plus fréquemment en attaque.**  
            Rappelons que lors d'un match de basketball, chaque équipe peut aligner simultanément 5 joueurs sur le terrain.  
            
            Il existe 5 postes au basketball, caractérisant l'espace et le style de jeu d'un joueur lorsqu'il attaque.  
            Cependant de nos jours, il n'est pas rare de voir des joueurs capables d'évoluer à 2 voire 3 postes différents (et même sur les 5 postes dans le cas de LeBron James !) : """)

    st.write("""
             
             - le **meneurs** (*point guard*) : c'est le joueur qui dirige l'attaque. Généralement le joueur le plus petit et le moins physique mais le plus vif de l'effectif, 
             ...
             - les **arrières** (*shooting guard*) : ils sont chargés de marquer des paniers par des tirs pris à l'extérieur de la ligne des 3 points, mais également parfois aussi de monter 
             ...
             - le **pivot** ou **intérieur** (*center*) : il est généralement reconnaissable au fait qu'il est le joueur le plus grand et le plus physique de l'effectif, mais également souvent 
                                                          le moins habile et le moins rapide.  
                                                          C'est lui qui est chargé de réaliser l'entre-deux au début des matchs. Son jeu se situe au plus proche du panier mais le plus souvent dos au panier, en se tenant près à "pivoter" lorsqu'il recevra le ballon.  
                                                          Des joueurs tels que ***Shaquille O'Neal***, ***Kareem Abdul Jabbar*** ou aujourd'hui ***Joel Embiid*** et ***Anthony Davis*** jouent au poste de pivot.  
             ...
             - les **ailiers forts** (*power forward*) : plus petits mais souvent plus puissants que les pivots, les ailiers forts sont également plus mobiles et plus habiles que ces derniers.  
                                                         Leur jeu est d'ailleurs assez similaire à celui des pivots puisque leur espace de j préférentiel se situe proche du panier, mais il diffère par le fait que l'ailier fort joue plutôt face au panier et non dos à celui-ci.  
             ...
             - les **ailiers** (*small forward*) : considérés comme les joueurs les plus polyvalents de l'effectif, leur puissance physique est un compromis entre celui des pivots et des ailiers forts tandis que leur agilité est un mix de celle des meneurs et des arrières.  
                                                   Ils sont tout autant capables de marquer sur des tirs longue distance à 3 points que sur des tirs dans la raquette proche du panier.  
    ...
    st.write(""" Suite aux rapides descriptions des 5 postes au basketball ci-dessus, il semble évident qu'un pivot et un meneur 
                 ...
                 Il serait donc osé et non productif de tenter de comparer les tirs de 2 joueurs jouant sur ces 2 postes, tout du moins en termes 
                 de fréquences de tirs par zones.""")

    st.header("Visualisations et analyses.")

    st.write("""**Choisissez le poste selon lequel vous souhaitez comparer les joueurs ainsi que l'élément de comparaison de ces joueurs** 
             (*le temps de chargement total pour l'affichage des cartes de tirs peut aller jusqu'à 30 secondes pour les meneurs et les arrières*) :""")
    ...
    st.write("")
    bouton_affichage = st.checkbox("Cocher cette case pour afficher les cartes de tirs")
    ...
    if bouton_poste == "meneurs":
        ...
        if bouton_elt_comparaison=="fréquence de tirs par zone":
            ...
            texte_a_afficher = """
                                On peut distinguer **3 catégories de meneurs** :  

                                **.** les meneurs de jeu **modernes des années 2010**, qui adorent prendre énormément de tirs au loin : c'est le cas de **Stephen Curry** et **Damian Lillard**.  
                                Ces joueurs font partie de la génération "[Moreyball](https://korii.slate.fr/et-caetera/moreyball-big-data-statistiques-mathematiques-nba-basket)", un style de jeu né dans les années 2010 et créé par l'ex-Manager général des Houston Rockets **Daryl Morey** consistant à monter la balle le plus vite possible vers l'avant avant que la défense ne soit repliée, afin de trouver un shoot ouvert au près du panier ou à 3 points.  
                                Dans cette philosophie de jeu, le tir à mi-distance est banni au profit du tir à 3 points, suite à un constat très simple réalisé par Morey : la probabilité de marquer un tir à mi-distance (donc à 2 points) n'étant pas très différente de la probabilité de marquer un tir pris derrière l'arc (donc à 3 points) mais les paniers à 2 points rapportant 1,5 fois moins de points que les paniers à 3 points (3/2 = 1,5), il est plus rentable de tenter sa chance à 3 points.  
                                Autrement dit, bien que la probabilité qu'un tir soit marqué diminue avec l'augmentation de la distance de tir, ce déficit de probabilité en faveur du tir à 2 points est largement compensé par le nombre de points que rapporte un panier à 3 points à l'équipe.  
                                Ce principe de jeu est parfaitement illustré par les Houston Rockets des années 2010, dont la figure de proue du projet se nommait James Harden.  
                                ...
                                Un autre phénomène intéressant réside dans l'élaboration par la NBA de règles visant à rendre ce sport spectaculaire depuis des années : et selon la NBA, cela passe par l'attaque.  
                                Ainsi, tout est fait pour que l'attaque soit au centre des matchs NBA, que le jeu soit rapide, que les séquences offensives se multiplient, avec par exemple l'évolution de la règle sur les rebonds offensifs il y a quelques saisons : si un tir est manqué et récupéré par un coéquipier (rebond offensif), l'équipe dispose de seulement 14 secondes (au lieu de 24 secondes auparavant) pour aller marquer un panier. L'objectif étant de ne pas voir une équipe s'éterniser et stagner en attaque.
                                
                                On voit donc bien la prédominance des tirs pris derrière l'arc (part de quasiment 40 % des tirs pour les deux joueurs). Notons cependant que les tirs à mi-distance sont assez fréquents, et loin d'être bannis comme on tente le faire chez les Rockets.  
                                ...
                                **.** les meneurs de jeu de **l'ancienne ère de la NBA** sont composés de **Allen Iverson**, **Steve Nash** et **Tony Parker** : les 2 premiers ont débuté en NBA lors de la saison 1996-1997, alors que Tony Parker n'a débuté qu'en 2021-2022. Leurs prises de tirs diffèrent grandement des 2 meneurs étudiés précédemment.  
                                ...
                                Cependant, Nash se distingue d'Iverson par une prise de tirs bien plus importante (quasiment le double d'Iverson) derrière l'arc des 3 points : la cause de cette observation est un principe précurseur du "Moreyball" appelé **"7 seconds or less"**. Ce style de jeu prône, comme son nom l'indique, une prise de tir rapide survenant moins de 7 secondes après qu'une équipe ait récupéré la balle, alors que la défense adverse n'est pas encore replacée. Steve Nash ayant joué pour les Suns de Phoenix durant une grande partie de sa carrière sous la houlette de Mike D'Antoni, l'entraîneur à l'origine de ce principe de jeu, il n'est pas étonnant de constater sa prise d'initiative importante à 3 points.  
                                Bien qu'appartenant à la même "génération", Tony Parker différait de ses 2 compatriotes : il prenait très peu de tirs derrière l'arc (seulement 6 % du temps), mais énormément au plus près du panier (zone restrictive, raquette) et à mi-distance. Curieusement, sa carte de tirs est très similaire à celle du pivot Anthony Davis, que nous avons décrypté précédemment, bien que ces 2 joueurs n'aient rien de comparable !  
                                
                                **.** les meneurs de **l'entre deux** sont ceux ayant joué en NBA entre les 2 périodes décrites ci-dessus. On y trouve un seul joueur : **Chris Paul**, qui aime prendre en priorité ses tirs à mi-distance (34 % de ses tirs), mais qui ensuite ne semble privilégier aucune zone puisqu'il prend des proportions assez similaires de ses tirs dans la zone restrictive, dans la raquette ou derrière l'arc des 3 points, ce qui démontre sa polyvalence et son absence de zone préférentielle.
                                
                                **.** Enfin, **Russell Westbrook** est un meneur de jeu moderne au jeu très atypique : bien qu'ayant une taille modeste (1.91 m) similaire à Steve Nash et Stephen Curry par exemple, son jeu très explosif voire même brutal favorisant le contact en fait un meneur bien différent des autres, comme en témoigne sa carte de tirs : assez peu de tirs tentés derrière l'arc des 3 points (seulement 18 %, à comparer aux 40 % de Lillard et Curry, les 2 autres joueurs de sa génération) mais énormément de tirs pris dans la zone restrictive, la raquette et à mi-distance.   
                                Son jeu tout en puissance le pousse à énormément porter le ballon et jouer sur son physique, même face à des joueurs au gabarit souvent beaucoup plus impressionnant que le sien, comme l'illustrent [les dunks impressionnants qu'il est capable de marquer](https://youtu.be/6YygTmWspyE). Nous pouvons même dire que son style de jeu est totalement opposé à celui de Stephen Curry.
            ...
            texte_a_afficher = """
                                ...
                                **. raquette** (*vert*) : **Chris Paul** est de loin le plus efficace dans la raquette, avec près de 49 % de réussite au tir dans cette zone. Ce pourcentage est même meilleur que celui des 3 pivots étudiés, qui tournaient autour de 43 % de réussite ici !  
                                ...
                                **. corner droit** : comme pour le corner gauche, **Stephen Curry** domine ses adversaires tant sur le nombre de tirs tentés que sur le taux de réussite dans ce coin, avec quasiment 50 % de réussite, loin devant Steve Nash et ses 40 %.  
                                Le mauvais élève se nomme une nouvelle fois Russell Westbrook, avec 29 % de réussite.
                                
                                """
            
        
        if bouton_affichage==True:
            ...
            for j in range(3):
                ...
                col1, col2 = st.columns(2)
                col1.write(figs[j+tour])
                col2.write(figs[j+tour+1])
        ...
        st.markdown("## <h1 style='text-align: center; color: black;'>ANALYSE :</h1>",
                    unsafe_allow_html=True)
        ...
        if bouton_affichage==True:
            ...
            # Affichage 1 à 1 des 3 cartes de tirs : 
        ...
        if bouton_elt_comparaison == "fréquence de tirs par zone":
            ...
            texte_a_afficher = """
                                ...
                                **. Ray Allen**, au coeur des débats sur le meilleur tireur à 3 points de l'histoire avec Stephen Curry, ne prenait pas tant de tirs derrière l'arc que ce que l'on peut penser (seulement 28 % derrière l'arc, contre 41 % pour Curry et Harden par exemple). 
                                Il tirait quasiment la même proportion de ses tirs à mi-distance, et est le joueur de la liste des 20 joueurs qui tentait le plus sa chance dans les corners avec une proportion de 10 %, corners gauche et droit confondus (pour la petite annecdote, un de ses tirs les plus célèbres reste [le panier marqué avec Miami dans le corner droit, à quelques secondes de la fin du match 6 des NBA Finals 2013 face aux Spurs de San Antonio](https://youtu.be/tr6XsZVb-ZE), ayant permi au Heat d'aller en prolongation puis de gagner ce match historique).  
                                ...
                                **. Manu Ginobili** possède une carte de tirs assez similaire à celle de James Harden : l'argentin privilégiait la prise de tir soit très proche du panier (zone restrictive, 1/3 du temps), soit à longue distance (derrière l'arc, quasiment 1/3 du temps). La raquette et la zone de mi-distance n'étaient pas délaissées, mais il y prenait beaucoup moins de tirs que dans les 2 premières zones citées.  
                                Derrière Allen et Curry, il est celui qui tente également la plus grande part de tirs dans les corners.  
                                
                                **. Dwyane Wade** possèdait des habitudes de tirs très différentes des autres arrières étudiés ici : sa carte de tirs ressemble de près à celle de Tony Parker (meneur). A l'intar de Manu Ginobili, il privilégiait 2 zones de tirs en particulier, mais pas les mêmes zones que Ginobili : les tirs très proches du panier (zone restrictive) et les tirs mi-distance.  
            ...
            texte_a_afficher =  """
                                ...
                                **BILAN :** Ray Allen est systématiquement dans le top 3 en terme d'efficacité : il est très précis quelque soit la zone du terrain dans laquelle il prend son tir, et ce n'est donc pas pour rien qu'il est considéré comme l'un des meilleurs tireurs de l'histoire de la NBA.  
                                A noter également que Kobe Bryant n'apparaît dans le top 3 que pour les zones de tirs proches du panier ou à mi-distance : il ne fait pas partie des meilleurs tireurs à 3 points, ce qui peut expliquer pourquoi il ne tentait pas une part faramineuse de ses tirs à 3 points.  
                                Enfin, Dwyane Wade est en dernière position dans tous les secteurs de tirs longue distance (corners + arc) : cela traduit des difficultés encore plus importantes que Kobe sur ce genre de tirs.
        ...
        if bouton_affichage==True:
            ...
            for i in range(3):
                ...
                col1, col2 = st.columns(2)
                col1.write(figs[i+tour])
                col2.write(figs[i+tour+1])
        ...
        if bouton_elt_comparaison=="fréquence de tirs par zone":
            ...
            texte_a_afficher = """
                                **. LeBron James** privilégie 2 zones de tir en particulier : la **zone restrictive** et la **zone mi-distance**. Au vue de son physique et de son style de jeu tout en puissance et en "enfoncement" du défenseur adverse, rien de très surprenant à cela puisqu'aucun joueur de la ligue n'est en mesure de le stopper une fois qu'il est lancé vers la panier, encore aujourd'hui à 37 ans !  
                                Malgré cela, les tirs derrière l'arc des 3 points sont loin de représenter une part négligeable de ses prises de tir, avec 20 % du total de ses tentatives. Notons cependant la dispersion des points représentant ses tirs derrière l'arc, pour certains très éloignés de l'arc !
            ...
            texte_a_afficher = """
                                ...
                                **BILAN :** On retrouve quasiment systématiquement le même top 3 et dans le même ordre : Kawhi Leonard - Kevin Durant - LeBron James.  
                                Giannis est quasiment en dernière position dans toutes les zones et présente de plus des pourcentages de réussite au tir assez éloignés de ceux de ses accolytes ! Même dans la zone restrictive, dans laquelle on rappelle qu'il prend 54 % de ses tirs, sont efficacité ne surpasse pas celle de James et Durant...  
                                Il est donc à la fois moins polyvalent, mais également moins précis que ses confrères ailiers forts !  
        ...
        if bouton_affichage==True:
            ...
            fig4 = plt.figure(figsize = (12.5,12.5))
            ax4 = fig4.add_subplot(111)
            ax4.set_facecolor("black")
    ...
    st.write("""
             ...
             faite par le modèle n’en serait que plus précise et proche de la réalité, et donc moins générale que si 
             nous considérions tous les joueurs dans un seul et même modèle.  
             ...
             par exemple est plutôt “mixte”, dans le sens où un joueur de ce poste peut tout aussi bien jouer et prendre ses tirs à 
             ...
             aurions-nous fait s’il notre entraîneur nous avait demandé de créer un modèle pour chaque joueur actuel de la ligue ?  
             Dans notre recherche d'efficience, en vue de réduire le nombre d’analyses à effectuer, le nombre 
             de modèles à développer et donc le temps de calcul de l’ordinateur ainsi que la mise à jour/la 
             maintenance des modèles, il est apparu plus logique de regrouper les joueurs jouant au même poste 
    ...
    jauge_nbr_lignes = st.slider(
        "Choisissez le nombre de lignes à afficher :", 5, 1000)
    ...
    st.write("""Afin de ne pas emcombrer la page d'une tonne de graphiques et d'explications et sachant que le même processus a été utilisé pour l'analyse des données de chaque groupe de joueurs,
                ...
                de l'analyse.  
                De plus, **les commentaires présents en guise d'analyse ne concernent que le groupe des ailiers/ailiers forts** (*LeBron James* et *Kevin Durant*).""")
    ...
    def distribution_cat(fig, ax, data = df , X = "shot made flag" , kind = "bar"):
        ...
        - module : le module de visualisation à utiliser pour tracer le graphique ('px' ou 'sns').
        
        - kind : le type de graphique à tracer ('pie' si module='px' , 'bar' si module = 'sns').
        ...
        if kind=="bar": # SI je souhaite tracer le diagramme en barres de la variable X :
            
            sns.countplot(data = data , x = X , order = data[X].value_counts().index, 
                          ax = ax) # afficher le diagramme en barres de la variable

            ax.set_title(f"Distribution de la variable '{X}'." , fontsize = 75 , 
                         family = "serif") # titre de la figure
            ax.set_xlabel(f"{X}" , fontsize = 42, family = "serif") # label de l'axe des abscisses
            ax.set_ylabel("effectif" , fontsize = 42, family = "serif") # label de l'axe des ordonnées
            ax.grid(alpha = 0.4 , ls = "--") # ajout d'une grille à la figure

            if len(data[X].value_counts().index)>3: # SI la variable prend strictement plus de 3 valeurs uniques : 

                rotation_labels = 60 # les textes des graduations de l'axe des abscisses sont écrits en biais

            else: # SI la variable prend 1, 2 ou 2 valeurs uniques seulement :

                rotation_labels = 0 # les textes des graduations de l'axe des abscisses sont écrits horizontalement

            # écriture des textes au niveau des graduations de l'axe des abscisses : 

            ax.set_xticks(ticks = range(len(data[X].value_counts().index))) 
                          
            ax.set_xticklabels(labels = data[X].value_counts().index , 
                               rotation = rotation_labels, fontsize = 30)


            # Ecriture des effectifs et des pourcentages de chaque barre : 
            
            for i in range(len(data[X].unique())): # Pour chaque valeur unique de la variable X :
                
                val = data[X].value_counts().index[i] # la valeur unique de la variable X
                effectif = data[X].value_counts().loc[val] # l'effectif de cette valeur unique
                txt1 = f"{effectif}"  # le texte à afficher au-dessus de la barre
                
                # affichage de ce texte au-dessus de la barre correspondant à la valeur unique : 
                
                ax.text(i, effectif+max(data[X].value_counts())/50 ,txt1 , fontsize = 35, horizontalalignment="center",
                         verticalalignment="center", color = "red")
                
                frequence = (data[X].value_counts(normalize = True).loc[val]*100).round(2) # la fréquence de cette valeur unique
                txt2 = f"({frequence}) %" # le texte à afficher dans la barre correspondante
                
                
                if (len(data[X].unique())<8) and (frequence>2): # SI la barre correspond à une valeur unique dont la fréquence dépasse 0,2 : 
                    
                    # Afficher le texte avec la fréquence d'apparition de la valeur dans la barre correspondante :
                    
                    ax.text(i, effectif/3, txt2, fontsize = 35 , color = "black", horizontalalignment="center", 
                             verticalalignment="center")
                

        else: # SI l'argument entré est différent de 'bar', retourner une erreur : 
            raise ValueError("valeur attendue pour l'argument 'kind ': 'bar'.")
    ...
    dico_traduction = {0:"tirs ratés" , 1:"tirs marqués"}
    
    graphe_cible = px.pie(values = data_groupe["shot made flag"].value_counts().sort_index(), 
                          names = dico_traduction, 
                          width = 600, 
                          height = 600, 
                          hole = 1/2, 
                          hover_name = dico_traduction,
                          color_discrete_map = {"tirs ratés":"red" , 
                                                "tirs marqués":"green"}, 
                          title = "Composition de la variable 'shot made flag'.")
    ...
    graphe_cat = distribution_cat(fig, 
                                  ax, 
                                  data=data_groupe,
                                  X=bouton_variable,
                                  kind="bar")
    ...
    col2.write(graphe_cat, width = 500)
    ...
    st.write("""
             ...
             ***- "home" :*** indique si le tir a été pris à domicile (home = 1) ou à l'extérieur (home = 0).  
              ...
              **.** 48,7 % des tirs ont été pris à domicile, 51,3 % à l'extérieur. Répartition peu près équilibré, sachant qu'en saison régulière, chaque équipe joue autant de matchs à domicile qu'à l'extérieur mais qu'en Playoffs (chaque tour se joue au meilleur des 7 matchs, nombre impair ==> une équipe jouera plus à domicile que l'autre), ce n'est pas forcément le cas.  
                    A noter également que les blessures éventuelles ou la non-utilisation de joueur font qu'un nombre plus important de matchs à domicile a pu être manqué.
             ...
             ***- "action type" :*** le type de tir utilisé par le joueur (dunk, layup, flotteur, step back, etc... : 11 types de tirs différents en tout).  
              ...
              **.** Présence d'une ambiguïté au sujet d'une modalité : le "jump shot" désigne le "tir en suspension", un tir très fréquent en NBA qui comporte plusieurs sous-catégories de tirs : "la layup, le step back, etc...  
                    Or ici, il a été considéré comme un type de tir indépendant et "atomique", car parmi les 10 autres catégories de tirs on trouve également "step back shot" , "layup shot" , etc..., ce qui signifie donc que selon de créateur du jeu de données, un "jump shot" et un "layup shot" par exemple ne sont pas les mêmes types de tirs, alors qu'en réalité les layups sont un sous-ensemble des jump shots...  
                    **MISE EN GARDE :** Variable ambigüe, **A ESSAYER DE NE PAS UTILISER LORS DE LA MODELISATION.**  
    ...
    def distribution_quant(fig, ax, data=df, X="time remaining", kind="hist", bins=60, range=(0, 12), xlim_inf_ecdf=-50):
        ...
        if kind == "hist":  # SI je souhaite représnter l'histogramme de X :
            ...
            ax.set_ylabel("effectif", fontsize=20, family = "serif")
            # titre du graphique
            ax.set_title(
                f"Histogramme de la variable '{X}'", fontsize=15, family="serif")
    ...
    st.write("""
              - Nos hypothèses selon lesquelles il existe une relation entre **coordonnée y/finalité du tir**, **distance de tir/finalité du tir** et **valeur du tir/finalité du tir** sont vérifiées par un test ANOVA : 
              ...
              - Cependant, le test **n'a pas permis de conclure à une influence statistiquement significative de "shot made flag" sur "x location"**, alors que nous avions supposé cette relation graphiquement.  
    ...
    st.write("""
             ...
             Afin d’y répondre, il faut se représenter visuellement les 2 cas de figures suivants en prenant l'exemple de Kevin Durant au tir :""")
    ...
    st.write("""           
            ...
            - **CAS 2 :** Supposons maintenant que **mon modèle prédit qu'un tir de Kévin Durant rentrera 
                ...
                *Quelle conséquence ?*
                ...
                de près et n'étant pas mauvais pour délivrer des passes décisives à ses équipiers par 
                ...
                camarades de jeu : je peux potentiellement encaisser pal mal de points “gratuitement” également dans cette situation !
                  
            
            
            
            **BILAN :** il faut donc à la fois que mon modèle ait **une bonne sensibilité ET une bonne spécificité :** 
            nous allons donc utiliser le **score F1** (moyenne harmonique du recall et de la precision) 
    ...
    X_train_piv, X_test_piv, y_train_piv, y_test_piv = data_preparation(groupe="pivots/ailiers forts")
    ...
    X_train_men, X_test_men, y_train_men, y_test_men = data_preparation(groupe="meneurs/arrieres")
    ...
    def ROC(y_true, y_score, type_modele, figsize = (8,5)):
        ...
        ax.plot(FP, TP, color="orange", lw=0.85, 
                label=f"{type_modele} (notre modèle)")
        
        
        ax.plot([0,1] , [0,1] , color = "blue" , ls = "--" , lw = 0.45)
        ax.plot([0,0,1] , [0,1,1] , color = "green" , ls = "--" , lw = 0.45)
        
        ax.set_xlabel("Taux de FAUX POSITIFS", fontsize = 6, family = "serif")
        ax.set_ylabel("Taux de VRAIS POSITIFS", fontsize = 6, family = "serif")
        ax.set_xticks(np.arange(0, 1.1, 0.1))
        ax.set_yticks(np.arange(0, 1.1, 0.1))
        ax.set_yticklabels(np.arange(0, 1.1, 0.1).round(2), fontsize = 5)
        ax.set_xticklabels(np.arange(0, 1.1, 0.1).round(2), fontsize = 5)
        
        
        ax.annotate(xy = (0.65,0.63) , text = "modèle aléatoire" , xytext = (0.72,0.525), 
                    fontsize = 5, color = "blue" ,
                    arrowprops = {"color":"blue" , "edgecolor" : "black" , 
                                  "headwidth":2.5, "width":0.15 , "headlength":2.5})
        
        
        ax.annotate(xy = (0.4,0.98) , text = "modèle parfait" , xytext = (0.15,0.85) ,
                    fontsize = 5, color = "green" ,
                    arrowprops = {"color":"green" , "edgecolor" : "black" , 
                                  "headwidth":2.5, "width":0.15 , "headlength":2.5})
    ...
    def make_prediction(entree, groupe="pivots/ailiers forts", modele=model_piv_ail):
        ...
        entree_finale = np.array(entree_finale).reshape(1, len(np.array(entree_finale)))
        
        # STANDARDISATION DU VECTEUR D'ENTREE (si nécessaire) : 
            
        if groupe in ["meneurs/arrieres" , "pivots/ailiers forts"] : 
            ...
            # standardisation des variables de X_train : 
    
            scaler = StandardScaler()

            scaler.fit(X_train) # entraînement sur le jeu d'entraînement
            
            # Application du StandardScaler au vecteur d'entrée : 
                
            entree_finale = scaler.transform(entree_finale)
    ...
    if bouton_groupe == "pivots/ailiers forts":
        ...
        st.write("""
                 - On peut remarquer que les variables ayant le plus d'importance dans les prédictions 
                   effectuées par le modèle sont celles qui réfèrent à la position relative du tireur par 
                   rapport au panier (plein axe, à gauche, excentré à gauche, ...) : étant donné le découpage 
                   de ces zones (voir partie BONUS de ce streamlit), nous pouvions nous y attendre.  
                   En effet, comme expliqué précédemment, les pivots et ailiers forts ont tendance à jouer au plus près du panier, au contact du cercle.
                   Ils sont donc régulièrement amenés à tirer dans l'axe et à faible distance du panier. 
                   Cependant, le fait de tirer de plus loin ou même en étant excentré s'avère plus difficile car ce genre de tirs nécessite 
                   un meilleur "touché", plus de finesse de la part du tireur , ce qui peut expliquer l'importance de la zone de tir.  
                
                - On constate cependant que le temps restant à jouer dans le quart-temps, le lieu du match ainsi que la phase de la saison (Davis et 
                  Antetokounmpo n'ont pas énormément joué les Playoffs dans leur carrière, surtout Anthony Davis) n'ont pas une très grande importance pour prévoir 
                  l'issue d'un tir pour ces 2 joueurs.
                  Cependant, en ce qui concerne le temps restant à jouer, celui-ci prend de l'importance lorsqu'il approhe de la toute fin du quart-temps (entre 0 et 1 minute restant à jouer) : 
                  la pression de marquer est probablement plus importante sur le tireur lorsqu'il reste très peu de temps à jouer, que lorsqu'il reste encore une chance de se rattraper en cas d'échec. """)
        ...
        st.write("""Choisissez vous-même le contexte de tir des pivots/ailiers forts, et regardez quelle serait l'issue du tir selon le modèle (la probabilité seuil permettant d'attribuer la classe 0 ou 1 à une entrée est fixée à **50 %**) :""")
        ...
        st.write("")
        st.write("**MISE EN GARDE :** bien qu'elles n'empêchent pas le modèle de tourner, certaines configurations sont impossibles 'dans la vie réelle' (ex : un tir pris **à 2 points** dans la **1ère moitié de terrain**)")
        ...
        prediction_entree = make_prediction(entree=entree, groupe="ailiers/ailiers forts",
                                            modele=model_ail_ail_forts)[0]
        
        
        
        probabilite = make_prediction(entree = entree , groupe = "ailiers/ailiers forts", 
                                modele = model_ail_ail_forts)[1]

        st.write(f"Probabilité qu'un tir pris dans ce contexte soit marqué : **{(probabilite[0,1]*100).round(2)} %**.")
    ...
    elif bouton_groupe=="meneurs/arrieres":  
        ...
        st.write("""
                 **DECRYPTAGE :**
                 ...
                 - Sur **6778 tirs marqués**, **4414** tirs (= 65 %) sont repérés par le modèle : 
                   autrement dit, **35 % des tirs marqués par Durant et James sont classés par 
                   le modèle comme étant des tirs râtés.**
                   Concrètement, cela signifie que dans 35 % des cas, pensant que l'adversaire ne 
                   marquera suite aux prévisions du modèle, nous passerons la consigne au défenseur 
                   de ne pas trop presser le tireur, qui en réalité marquera.
                   
                 - Sur **8168 tirs prédits marqués** par le modèle, **4414** (= 54 %) sont réelement des tirs ayant été 
                   marqués : autrement dit, **46 % des tirs prédits marqués par le modèle sont en fait des 
                   tirs ayant été râtés.**
                   Concrètement, cela signifie que dans 46 % des cas, pensant que le tireur marquera 
                   suite aux prévisions du modèle, nous demanderons au défenseur de mettre une 
                   pression maximale sur le tireur voire même à un autre défenseur de venir en soutien, alors qu'en réalité, 
                   le tireur ne marquerait pas dans cette situation : ce qui lui laisse une occasion de déborder son vis-à-vis ou de 
                   délivrer une passe décisive à un équipier libre de tout marquage. 
        ...
        st.write("""
                 - Comme évoqué à de nombreuses reprises lors de ce projet, la variable "action type" possède une interprétation ambigüe à cause de sa modalité "jump shot".  
                   De plus, un dunk est le tir ayant la plus forte probabilté d'être marqué au basketball, au point que voir un joueur rater un dunk relève du ridicule. Autrement dit, savoir qu'un joueur 
                   a tenté un dunk nous révèle avec quasi-certitude que le tir sera marqué, quel que soit le joueur en général.  
                   Nous avions déjà visualisé, lors des phases de tests des différents modèles, l'importance énorme des modalités "jump shot" et "dunk shot" dans les prévisions du modèle : elles ont une si grande  
                   importance qu'elles écrasent totalement le poids des autres variables, au point de les réduire à zéro !  
                   
                   Bien que nous soyons parvenu à éviter son usage dans les 2 autres modèles, il n'a pas été simple de trouver de la performance porur le modèle des meneurs, 
                   ce qui explique la présence de cette variable parmi les variables explicatives du modèle : elle contribue certes grandement aux assez bonnes performances 
                   de ce modèle en termes de recall et de précision, mais en revanche, cela se paie par une interprétation ainsi que des prédictions qui peuvent être erronées !""")
        ...
        st.write("""**Pour des raisons d'interprétation, au vu des résultats obtenus lors du développement de ce modèle et afin de ne pas induire en erreur les utilisateurs, 
                    aucun générateur de prédiction du modèle n'est développé ici à l'instar des 2 autres modèles.**""")
    ...
    st.write("""
            N'existant aucune étude sur la prévision de tirs NBA utilisant ne cherchant à optimiser le F1-score( en général, c'est l'accuracy qui est choisie), 
            il est difficile de situer la performance de nos modèles par rapport à ceux développés dans ces études.  
            
          Nous ne pouvons alors que nous référer à notre objectif initial, qui était d’obtenir un bon compromis entre le recall et 
          la précision afin de juger la performance de nos 3 modèles, en nous posant la question suivante : ***serions-nous prêt à déployer et utiliser nos 
          ...
          - Pour les modèles du groupe des **ailiers/ailiers forts** et du groupe des **meneurs/arrières**, la réponse à cette question serait probablement 
            **NON.**  
            Bien que les 2 modèles présentent un bon compromis entre les 2 métriques (qui sont assez proches et avoisinent les 60 %), 
            les valeurs obtenues pour le recall (63 % pour les ailiers/ailiers forts et 65 % pour les meneurs/arrières) ne sont 
            à notre sens pas suffisamment élevées, ce qui fait que trop de tirs ayant été réellement marqués sont mal classés par nos 
            modèles, engendrant alors dans cette situation une erreur aux grosses conséquences car nous aurons tendance à "laisser tirer" l'adversaire suite à la préduiction de notre modèle, 
            ce qui aboutira à l'encaissement de "points faciles" que l'on aurait pu éviter.  
            De plus, le modèle des meneurs/arrières utilise parmi ses variables explicatives la variable “action type”, dont 
            l’interprétation de la modalité “jump shot” par le créateur du jeu de données porte à confusion et dont l'importance dans les prévisions effectuées par le modèle est énorme, come nous l'avons vu 
            dans la partie 4) sur la modélisation. Il en résulte que les prévisions effectuées par ce modèle sont sûrement beaucoup moins fiables que celles effctuées par les 2 autres modèles, et il paraît assez
            risqué de l'utiliser à des fins pédictives.
        ...
        Antetokounmpo et Anthony Davis) que chez les autres groupes de joueurs : il faut savoir qu’en NBA, les pivots sont 
        des joueurs ayant généralement vocation à jouer à l’intérieur de la ligne des 3 points, autour du panier. Ainsi, leur 
        espace de jeu et leurs lieux de tirs préférentiels sont beaucoup moins denses que ceux des meneurs, des ailiers et des 
        arrières. De plus, nous avons clairement vu lors de l’analyse des 20 joueurs que Giannis, bien qu’évoluant au poste 
        ...
        lesquelles soit le joueur marque (la plupart du temps), soit il rate son tir.
        Ce n'est en revanche pas aussi simple pour les joueurs évoluant aux autres postes, beaucoup plus complets et polyvalents que des pivots et pour lesquels prévoir l’issue d'un tir nécessite d'avoir des informations plus nombreuses 
        et plus précises que celles apportées par les 20 variables de ce jeu de données.
        
        En résumé, excepté peut-être le modèle des ailiers forts/pivots, ces modèles ne pourraient probablement pas être
        déployés “dans la vie réelle”, sous peine de prendre des décisions menant à commettre trop d’erreurs préjudiciables à l’équipe.""")  
    ...
    st.write("""
              Nous pouvons alors nous demander la chose suivante : ***Si nous avions disposé de 6 mois supplémentaires pour développer nos modèles, qu'aurions-nous pu chercher à
              ...
              variables supplémentaires créées de toute pièce, sans forcément chercher à obtenir d’autres données issues d’autres 
              jeux.  
    ...
    st.write("""  
            ...
            - le **niveau défensif global de l’équipe adverse :** comme évoqué plusieurs fois auparavant, le jeu actuel de la NBA étant particulièrement porté vers l'attaque, la plupart des équipes sont plutôt dans l'optique de "marquer plus de points que l'adversaire" 
              plutôt que "d'encaisser moins de points que l'adversaire".  
              Autrement dit, certaines équipes ont tendance à délaisser voire sacrifier l'aspect défensif du jeu au profit de l'optimisation du jeu offensif.   
            ...
            - **savoir si l'équipe adverse est une franchise dans laquelle le tireur à déjà joué au cours de sa carrière on pas :** à l'instar du classement de l'adversaire, certains joueurs jouent bien mieux contre une équipe par laquelle ils sont passés dans leur carrière car ils ont à coeur de montrer à leurs anciens dirigeants et équipiers ce qu'ils valent et ce 
              qu'ils sont capables d'accomplir dans une autre franchise, tandis que d'autres ont tendance à "surjouer" et à vouloir faire leurs preuves plus que contre n'importe quelle autre équipe, au point de ne pas jouer naturellement, de déjouer et de voir leurs performances chuter face à cet adversaire.
          
            Ces informations n'étant pas présentes dans le jeu de données utilisé, nous aurions sans doute pu pratiquer le web scraping sur [le site officiel de la NBA](https://www.nba.com/stats/teams/traditional/?sort=W_PCT&dir=-1),
    ...
    st.write("""Vous souhaitez faire vos propres comparaisons de joueurs ?  
                Vous avez ici la possibilité de comparer directement les cartes de tirs de 2 joueurs au choix parmi 
                une liste élargie de joueurs, dont certains ne faisaient pas partie de ce projet :""")
    ...
    if bouton_joueur_1=="kentavious caldwell-pope" or bouton_joueur_2=="kentavious caldwell-pope":
    ...
    if bouton_joueur_1!="" and bouton_joueur_2!="":
        ...
        if bouton_season_type == "mixte":
            ...
            if bouton_home == "mixte":
                ...
                if bouton_adv == "tous":
                    ...
                    if (adversaries[bouton_adv] in df_elargi[df_elargi["player name"] == bouton_joueur_1]["adversary"].unique()) and (adversaries[bouton_adv] in df_elargi[df_elargi["player name"] == bouton_joueur_2]["adversary"].unique()):
    
                        trace1 = shot_chart(fig1, ax1, figsize=figsize,
                                            data=df_elargi, critere="adversary",
                        ...
                        if adversaries[bouton_adv] not in df_elargi[df_elargi["player name"] == bouton_joueur_1]["adversary"].unique():
                ...
                if bouton_adv == "tous":
                    ...
                    if (dico_home[bouton_home] in df_elargi[(df_elargi["player name"] == bouton_joueur_1) & (df_elargi["adversary"] == adversaries[bouton_adv])]["home"].unique()) and (dico_home[bouton_home] in df_elargi[(df_elargi["player name"] == bouton_joueur_2) & (df_elargi["adversary"] == adversaries[bouton_adv])]["home"].unique()):
    
                        trace1 = shot_chart(fig1, ax1, figsize=figsize,
                                            data=df_elargi[(df_elargi["home"] == dico_home[bouton_home]) & (
                                                df_elargi["adversary"] == adversaries[bouton_adv])],
                                            player=bouton_joueur_1,
                                            frequency_or_efficiency=dico_pct[bouton_chiffres],
                                            hue=dico_hue[bouton_hue])
                        ...
                        if dico_home[bouton_home] not in df_elargi[(df_elargi["player name"] == bouton_joueur_1) & (df_elargi["adversary"] == adversaries[bouton_adv])]["home"].unique():
            ...
            if bouton_home == "mixte":
                ...
                if bouton_adv == "tous":
                    ...
                    if (dico_season_type[bouton_season_type] in df_elargi[df_elargi["player name"] == bouton_joueur_1]["season type"].unique()) and (dico_season_type[bouton_season_type] in df_elargi[df_elargi["player name"] == bouton_joueur_2]["season type"].unique()):
                        ...
                        if dico_season_type[bouton_season_type] not in df_elargi[df_elargi["player name"] == bouton_joueur_1]["season type"].unique():
                    ...
                    if (adversaries[bouton_adv] in df_elargi[(df_elargi["player name"] == bouton_joueur_1) & (df_elargi["season type"] == dico_season_type[bouton_season_type])]["adversary"].unique()) and (adversaries[bouton_adv] in df_elargi[(df_elargi["player name"] == bouton_joueur_2) & (df_elargi["season type"] == dico_season_type[bouton_season_type])]["adversary"].unique()):
    
                        trace1 = shot_chart(fig1, ax1, figsize=figsize,
                                            data=df_elargi[(df_elargi["season type"] == dico_season_type[bouton_season_type]) & (
                                                df_elargi["adversary"] == adversaries[bouton_adv])],
                                            player=bouton_joueur_1,
                                            frequency_or_efficiency=dico_pct[bouton_chiffres],
                                            hue=dico_hue[bouton_hue])
                        ...
                        if adversaries[bouton_adv] not in df_elargi[(df_elargi["player name"] == bouton_joueur_1) & (df_elargi["season type"] == dico_season_type[bouton_season_type])]["adversary"].unique():
                ...
                if bouton_adv == "tous":
                    ...
                    if (dico_home[bouton_home] in df_elargi[(df_elargi["player name"] == bouton_joueur_1) & (df_elargi["season type"] == dico_season_type[bouton_season_type])]["home"].unique()) and (dico_home[bouton_home] in df_elargi[(df_elargi["player name"] == bouton_joueur_2) & (df_elargi["season type"] == dico_season_type[bouton_season_type])]["home"].unique()):
    
                        trace1 = shot_chart(fig1, ax1, figsize=figsize,
                                            data=df_elargi[(df_elargi["home"] == dico_home[bouton_home]) & (
                                                df_elargi["season type"] == dico_season_type[bouton_season_type])],
                                            player=bouton_joueur_1,
                                            frequency_or_efficiency=dico_pct[bouton_chiffres],
                                            hue=dico_hue[bouton_hue])
                        ...
                        if dico_home[bouton_home] not in df_elargi[(df_elargi["player name"] == bouton_joueur_1) & (df_elargi["season type"] == dico_season_type[bouton_season_type])]["home"].unique():
                    ...
                    if (bouton_adv in df_elargi[(df_elargi["player name"] == bouton_joueur_1) & ((df_elargi["season type"] == dico_season_type[bouton_season_type]) & (df_elargi["home"] == dico_home[bouton_home]))]["adversary"].unique()) and (bouton_adv in df_elargi[(df_elargi["player name"] == bouton_joueur_2) & ((df_elargi["season type"] == dico_season_type[bouton_season_type]) & (df_elargi["home"] == dico_home[bouton_home]))]["adversary"].unique()):
    
                        trace1 = shot_chart(fig1, ax1, figsize=figsize,
                                            data=df_elargi[((df_elargi["home"] == dico_home[bouton_home]) & (df_elargi["adversary"] == adversaries[bouton_adv])) & (
                                                df_elargi["season type"] == dico_season_type[bouton_season_type])],
                                            player=bouton_joueur_1,
                                            frequency_or_efficiency=dico_pct[bouton_chiffres],
                                            hue=dico_hue[bouton_hue])
                        ...
                        if bouton_adv not in df_elargi[(df_elargi["player name"] == bouton_joueur_1) & ((df_elargi["season type"] == dico_season_type[bouton_season_type]) & (df_elargi["home"] == dico_home[bouton_home]))]["adversary"].unique():
