...
if pages == "Petite initiation à la NBA":
    ...
    st.markdown("""Une autre particularité de la NBA est le fait que le **calendrier des matchs d'une franchise est asymétrique** : bien que chaque franchise joue le même nombre de matchs que les autres, le nombre de matchs joués contre chaque adversaire diffère d'une franchise à une autre.  
                   En effet, afin d'éviter en permanence des déplacements longs et coûteux pour jouer des matchs à l'autre bout des Etats-Unis, les 30 franchises sont réparties selon leur situation géographique dans **2 conférences** (conférence **Ouest** et conférence **Est**, comptant chacune **15 franchises**) 
                   et dans **6 divisions** (chaque conférence comptant **3 divisions de 5 franchises**).  
                   Pour plus de clarté, voici une image résumant la situation :""",
                unsafe_allow_html=True)
    ...
    st.write("""
            ...
            **.** *b) Recherche de données aberrantes.*  
            ...
            d’entraînement choisies en 5-1) et 5-2), par validation croisée.  
    ...
    df_elargi = df_elargi[df_elargi["Player Name"].isin(players)]
    ...
    st.dataframe(df_elargi.head(curseur_data))

    st.write(f"""
             - lignes : **{len(df_elargi)}**  
             - colonnes : **{df_elargi.shape[1]}**  
             - valeurs manquantes : **{df_elargi.isna().sum().sum()}**""")
    ...
    bouton_affichage = st.checkbox("Cocher pour afficher les cartes de tirs")
